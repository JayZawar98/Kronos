...
def get_portfolio() -> dict:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT cash_balance FROM portfolio WHERE id = 1")
        row = cursor.fetchone()
        if row:
            cash_balance = row[0]
            
            # Calculate currently invested capital
            cursor.execute("SELECT SUM(quantity * entry_price) FROM positions")
            invested_row = cursor.fetchone()
            invested_capital = invested_row[0] if invested_row and invested_row[0] else 0.0
            
            total_equity = cash_balance + invested_capital
            starting_capital = 10000.0
            net_profit = total_equity - starting_capital
